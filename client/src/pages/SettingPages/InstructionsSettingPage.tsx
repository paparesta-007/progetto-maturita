...
const InstructionsSettingPage: React.FC = () => {
    ...
    const styles = {
        ...
        input: `w-full border rounded-lg px-3 py-2 text-sm outline-none transition-all ${isDark
    ...
    return (
        ...
        <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className={styles.footer}>
                <button onClick={handleSend} className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-medium transition-all shadow-sm ${isDark ? "bg-white text-neutral-900 hover:bg-neutral-200" : "bg-neutral-900 text-white hover:bg-neutral-800"
                    }`}>
...
const InputGroup = ({ label, placeholder, value, onChange, isDark }: any) => (
    <div className="flex flex-col gap-1.5">
        ...
        <input
            ...
            className={`w-full border rounded-lg px-3 py-2 text-sm outline-none transition-all ${isDark ? "bg-neutral-900 border-neutral-700 text-white" : "bg-white border-neutral-300 text-neutral-900"
                }`}
...
const ToggleSwitch = ({ active, onClick, isDark }: { active: boolean, onClick: () => void, isDark: boolean }) => (
    <button
        onClick={onClick}
        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${active ? (isDark ? 'bg-white' : 'bg-neutral-900') : (isDark ? 'bg-neutral-700' : 'bg-neutral-200')
            }`}
    >
